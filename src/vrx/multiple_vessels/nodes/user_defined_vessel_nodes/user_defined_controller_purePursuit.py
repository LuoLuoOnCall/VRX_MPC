...
if __name__ == '__main__':
    ...
    for obj in vessel_objects:
        obj.start()
'''
std::pair<double, double> RRT::getLocalCoord(double lat, double lon) {
    // 常量定义
    const double WORLD_POLAR_M = 6356752.3142;
    const double WORLD_EQUATORIAL_M = 6378137.0;
    
    // 计算地球椭球体参数
    double eccentricity = std::acos(WORLD_POLAR_M / WORLD_EQUATORIAL_M);
    double latRad = lat * M_PI / 180.0; // 转换为弧度
    double sinLatSquared = std::pow(std::sin(latRad), 2.0);
    double sinEccSquared = std::pow(std::sin(eccentricity), 2.0);
    double n_prime = 1.0 / std::sqrt(1.0 - sinLatSquared * sinEccSquared);
    double m = WORLD_EQUATORIAL_M * std::pow(std::cos(eccentricity), 2.0) * std::pow(n_prime, 3.0);
    double n = WORLD_EQUATORIAL_M * n_prime;
    
    // 计算经纬度差值
    double diffLon = lon - origin_lon;
    double diffLat = lat - origin_lat;
    
    // 计算表面距离
    double surfdistLon = M_PI / 180.0 * std::cos(latRad) * n;
